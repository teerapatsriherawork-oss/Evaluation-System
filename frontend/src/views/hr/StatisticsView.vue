      ...
      <v-table density="compact" class="bg-transparent">
        ...
        <tbody>
          <tr v-for="(s,i) in stats.evaluatee_scores" :key="i">
            ...
            <td><v-chip size="x-small" :color="gradeOf(s.avg_score).color" label>{{ gradeOf(s.avg_score).label }}</v-chip></td>
          </tr>
...
const renderCharts = () => {
  ...
  if (chartRef.value && es.length) {
    chartInst = new Chart(chartRef.value, {
      type: 'bar',
      data: { labels: es.map(e => e.full_name), datasets: [{ label: 'คะแนนเฉลี่ย', data: es.map(e => Number(e.avg_score).toFixed(2)), backgroundColor: 'rgba(99,102,241,0.6)', borderColor: '#6366f1', borderWidth: 1, borderRadius: 8 }] },
  ...
  if (topicChartRef.value && ts.length) {
    ...
    topicChartInst = new Chart(topicChartRef.value, {
      type: 'doughnut',
      data: { labels: ts.map(t => t.topic_title), datasets: [{ data: ts.map(t => Number(t.avg_score).toFixed(2)), backgroundColor: colors.slice(0, ts.length), borderWidth: 0 }] },
...
const fetchStats = async () => {
  ...
  try {
    const { data } = await api.get(`/reports/statistics/${periodId.value}`)
...
onMounted(async () => {
  ...
  if (p.length) { periodId.value = p[0].id; await fetchStats() }
})
</script>
